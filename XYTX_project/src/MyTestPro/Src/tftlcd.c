...
void LCD_ShowPicture(u16 x, u16 y, u16 wide, u16 high,const u8 *pic)
{
	u16 temp = 0;
	long tmp=0,num=0;
...
void LCD_ShowCharColor(u16 x,u16 y,u8 ch,u8 size,u16 FG_Color,u16 BG_Color)
{
	u8 px=0;//字库的x坐标
	u8 py=0;//字库的y坐标
	u16 y0=y;//保存y初始值
	...
	ch=ch-' ';
	//获取第一个字节
	for(px=0;px<csize;px++)
	{
		//找到该字符所在字库的位置
		if(size==12)cTemp=asc_1206[ch][px]; 	 	//调用1206字体
		else if(size==16)cTemp=asc_1608[ch][px];	//调用1608字体
		else if(size==24)cTemp=asc_2412[ch][px];	//调用2412字体
		else return;
		for(py=0;py<8;py++)
		{			    
			if(cTemp&0x80)LCD_DrawFRONT_COLOR(x,y,FG_Color);
			else LCD_DrawFRONT_COLOR(x,y,BG_Color);
			cTemp<<=1;
			y++;
...
void LCD_ShowNumColor(u16 x,u16 y,u32 num,u8 len,u8 size,u16 FG_Color,u16 BG_Color)
{
		u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowCharColor(x+(size/2)*t,y,' ',size,FG_Color,BG_Color);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowCharColor(x+(size/2)*t,y,temp+'0',size,FG_Color,BG_Color); 
	}
}
...
void LCD_ShowFontHZ(u16 x,u16 y,u8 *cn,u16 FG_Color,u16 BG_Color)
{
	u8 px,py,WordNum;
	char ch1;
	char ch2;
	...
	while(*cn !='\0')
	{
		...
		{
			if((CnChar24x24[WordNum].Index[0]==*cn)&&(CnChar24x24[WordNum].Index[1]==*(cn+1)))
			{
				pos=(char *)CnChar24x24[WordNum].Msk;
				for(px=0;px<24;px++)
				{
					//MSK的位数
					ch1=*pos++;
					...
					for(py=0;py<8;py++)
					{
						if(ch1&(1<<(7-py)))
						{
							LCD_DrawFRONT_COLOR(x+px,y+py,FG_Color);
						}
						else
						{
							LCD_DrawFRONT_COLOR(x+px,y+py,BG_Color);
						}
						if(ch2&(1<<(7-py)))
						{
							LCD_DrawFRONT_COLOR(x+px,y+py+8,FG_Color);
						}
						else
						{
							LCD_DrawFRONT_COLOR(x+px,y+py+8,BG_Color);
						}
						if(ch3&(1<<(7-py)))
						{
							LCD_DrawFRONT_COLOR(x+px,y+py+16,FG_Color);
						}
						else
						{
							LCD_DrawFRONT_COLOR(x+px,y+py+16,BG_Color);
						}
					}//for（py<0;py<8;py++）结束
				}
			}
...
void LCD_ShowString(u16 x,u16 y,char* str,u8 size,u16 FG_Color,u16 BG_Color)
{
	int iCount=0;
	unsigned int lenth;
	lenth=strlen(str);
	for(iCount=0;iCount<lenth;iCount++)
	{
		 LCD_ShowCharColor(x,y,*str,size,FG_Color,BG_Color);
		 str++;
			x+=size/2;
	}
}
...
void Free()
{
	...
	LCD_Fill(3,373,27,397,GREEN);
	LCD_ShowFont12Char(30,376,"空闲状态",GREEN,BLACK);
}

void Busy( uint8_t *cn)
{
	LCD_Fill(0,370,239,399,BLACK);
	LCD_Fill(3,373,27,397,RED);
	LCD_ShowFont12Char(30,376,cn,RED,BLACK);
}
