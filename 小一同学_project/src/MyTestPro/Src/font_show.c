...
void LCD_ShowFont12Char(uint16_t x, uint16_t y, uint8_t *ch,u16 fg_color,u16 bg_color)
{
        ...
        if(*ch < 0x80)  //ASCII码从0~127
        {
    		...
    		for (j=0; j<16; j++) //每个字模一共有16个字节
    		{
    			...
    			for (i=0; i<8; i++) 
    			{
    				if ((color&0x80) == 0x80)
    				{
    					LCD_WriteData_Color(fg_color);
    				} 						
    				else
    				{
    					LCD_WriteData_Color(bg_color);
    				} 	
    				color <<= 1;
    			}
    		}
...
void LCD_ShowFont16Char(uint16_t x, uint16_t y, uint8_t *cn ,u16 fg_color,u16 bg_color)
{   
